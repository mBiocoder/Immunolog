...
FeaturePlot(skin_fetus_and_adult_data, features = c("IL1A", "IL1B"), split.by = "orig.ident", min.cutoff = 'q10')

#Dotplot to investigate IL1B expressing clusters 13, 16 and 17 closer
...
#Subset to remove skin-fetus clusters 17, 16, 13
testing <- subset(x = skin_fetus_and_adult_data, idents = c("13", "16", "17"), invert = TRUE)
FeaturePlot(testing, features = c("IL1B"), split.by = "orig.ident", min.cutoff = 'q10')

# Re-visualize the clusters separated by orig.ident
DimPlot(object = testing, 
        reduction = "umap", 
        label = TRUE,
        label.size = 3,
        repel = TRUE, split.by = "orig.ident")

#DimPlot(skin_fetus_and_adult_data, reduction = "umap", dims = c(1,2), split.by = "orig.ident", label = T, label.size = 6, pt.size = 0.8, cols = NULL) 
#DimPlot(skin_fetus_and_adult_data, reduction = "pca", split.by = "orig.ident", label = T, label.size = 6, pt.size = 0.8) 
...
#Plot separate heatmaps for out op 50 up -and 50 downregulated DEGs
DoHeatmap(skin_fetus_and_adult_data, features = rownames(deg.gene.pos)) #upregulated
DoHeatmap(skin_fetus_and_adult_data, features = rownames(deg.gene.neg)) #downregulated
