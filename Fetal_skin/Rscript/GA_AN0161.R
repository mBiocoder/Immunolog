...
# - manual cluster annotation based on cluster markers educated by singleR results
#=================================================================================================================#
#=================================================================================================================#
# Prepare R session
#=================================================================================================================#
...
an.desc <- c("sc_skin_blood_10x_ann")
an.descs <- c("sc_skin")
...
treg <- c('CTLA4', 'TIGIT', 'ICOS') #cluster 9
cd4 <- c('') #clusters 0,1,2,3,5
cd8 <- c('CD8A', 'CD8B') #clusters 4,6,7,11,12
cd8.4 <- c('EOMES', 'CST7', 'TIGIT', 'PDCD1', 'CXCR6', 'RASGRP1', 'PRF1', 'LAG3') # cluster 4 cd8+ eomes+ CST7+ tigit+ pd1+ cxcr6+ RASGRP1+ (GZMA/K/H, CCL3/4/5, HLA-DPB1/DRB1, NKG7)
#https://www.ncbi.nlm.nih.gov/pubmed/24823625 (CXCR6, development and maintenance of memory T cell)
#http://www.jimmunol.org/content/190/1_Supplement/124.12 (CXCR6, lung residency)
#https://www.ncbi.nlm.nih.gov/pubmed/20007535 (RASGRP1, regulates the threshold of T cell activation, through IL2 production)
#https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4251435 (CST7, regulates GZM expression by inhibiting the protease that generate active GZM from precursors)
#https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5835137/ (LAG3, expressed in activated T cells, its blockade enhances immune response)
cd8.12 <- c('CST7', 'LITAF', 'IFNG', 'TNF') # cluster 12 cd8+ eomes+ CST7+ tigit- pd1- cxcr6- LITAF- (GZMA/K/H, CCL3/3L1/4/4L2/5, HLA-DPB1/DRB1, NKG7, HSPA6/A1A/A1B/90AA1, DNAJB4)
#https://www.ncbi.nlm.nih.gov/pubmed/24162776 (LITAF, TF upregulated in Trm)
#http://www.jimmunol.org/content/jimmunol/178/9/5690.full.pdf (JUN, regulates lymphocyte development by repressing IL7Ra promoter, favors gdT formation)
#https://cvi.asm.org/content/8/3/628 (IFNG, marker of citotoxicity)
cd8.6 <- c('IL2RB', 'LDHA', 'LITAF', 'CHPT1', 'UBE2F', 'TRGC2', 'CD99', 'CD55', 'XCL1', 'SURF4', 'CAST', 'PGK1', 'TRDC', 'CD7', 'CD63', 'JAML', 'IL32', 'CD9', 'GNLY') # cluster 7 hobit- 
cd8.7 <- c('IL2RB', 'LDHA', 'LITAF', 'CHPT1', 'UBE2F', 'TRGC2', 'CD99', 'CD55', 'XCL1', 'SURF4', 'CAST', 'ZNF683', 'LMNA', 'EZR', 'PTP4A1', 'MCL1', 'REL') # cluster 6 hobit+ 
#https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4295558 (IFITM2, antiviral activity)
#https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4693623 (JAML, cd8 migration to the brain)
#http://www.jimmunol.org/content/jimmunol/170/5/2349.full.pdf (CD7, high in naive and memory, low or negative in effector)
#http://www.jimmunol.org/content/jimmunol/173/10/6000.full.pdf (CD63, reinforces activation and increases response to restimulation)
#https://www.ncbi.nlm.nih.gov/pubmed/9278313 (CD99, regulates ICAM-1-mediated cell adhesion process)
#https://www.nature.com/articles/emm200720.pdf?origin=ppub (CD99, co-stimulation of T cells)
#https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2192734 (CD9, CD28- independent costimulatory signal on T cells)
gd <- c() #cluster 8
ilc <- c() # cluster 10
#rownames(sk@assays[["RNA"]])[grep("TRD|TRG", rownames(sk@assays[["RNA"]]))] #all gdT-related genes

# Visualization of marker gene expression (tSNE)
# when the violin plot shows no expression, because it is not expressed in the majority of cells in the cluster, dots could be added (using ifelse)
vp.mk.sk <- sapply(as.numeric(levels(sk@active.ident))+1, function(x) NULL)
for(i in 0:max(as.numeric(as.character(sk@active.ident)))) {
  df.sk <- data.frame(cluster = sk@active.ident, 
                      t(as.matrix(GetAssayData(sk, slot = "data")[cd8,])))
  ...
  vp.mk.sk[[i+1]] <- ggplot(df.sk, aes(x = cluster, y = value, fill = cluster)) + 
    ...
    geom_jitter(shape = 16, size = 0.5, alpha = 0.5, position=position_jitter(0.2), color = "black", show.legend = F) + 
